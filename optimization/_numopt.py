...
def steepest_descent(fun, grad, x0, iterations, tol):
    ...
    while (gnorm > tol) and (k < iterations):
        pk = -gfk
        try:
            alpha, fc, gc, old_fval, old_old_fval, gfkp1 = _line_search_wolfe12(fun, grad, xk, pk, gfk, old_fval,
                                                                                old_old_fval, amin=1e-100, amax=1e100)
...
def levenberg_marquardt(fun, grad, jacobian, x0, iterations, tol):
    ...
    -------
    xk : numpy.array, size=9
        Parameters estimated by optimization algorithms.
    fval : float
        Objective function value at xk.
    grad_val : float
        Gradient value of objective function at xk.
    ...
    updateJ = 1
    lamda = 0.01  # 迭代步长：（1）下降太快时使用较小的值；（2）下降太慢时，使用较大的值。
    old_fval = fun(x0)
    gfk = grad(x0)
    ...
    while (gnorm > tol) and (k < iterations):
        ...
        gfk = grad(xk)
        pk = - np.linalg.inv(H_lm).dot(gfk)  # pk：search direction
